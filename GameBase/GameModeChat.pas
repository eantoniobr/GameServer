...
constructor TGameChat.Create(const PL: TClientPlayer; GameData: TGameInfo;
  CreateEvent, UpdateEvent, DestroyEvent: fGameEvent;
  OnJoin, OnLeave: fPlayerEvent; GameID: UInt16);
begin
...
function TGameChat.GameInformation: AnsiString;
var
...
procedure TGameChat.OnPlayerLeave;
begin
  inherited;
  { This chat class is do nothing }
end;

procedure TGameChat.SendPlayerOnCreate(const PL: TClientPlayer);
var
  Packet: TClientPacket;
begin
  inherited;
...
procedure TGameChat.SendPlayerOnJoin(const PL: TClientPlayer);
var
  Packet: TClientPacket;
  P: TClientPlayer;
begin
...
function TGameChat.Validate: Boolean;
begin
  Exit(True);
end;
